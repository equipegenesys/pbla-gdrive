...
user_tag_table = Table('user_tag_association', Base.metadata,
    Column('pblacore_uid', Integer, ForeignKey('users.pblacore_uid')),
    Column('pblacore_tag', String, ForeignKey('tags.pblacore_tag'))
)

user_file_table = Table('user_file_association', Base.metadata,
    Column('pblacore_uid', Integer, ForeignKey('users.pblacore_uid')),
    Column('local_fileid', Integer, ForeignKey('files.local_fileid'))
)
...
class User(Base):
    ...
    tags = relationship("Tag", secondary=user_tag_table, back_populates="users")

    files = relationship("File", secondary=user_file_table, back_populates="users")
...
class Tag(Base):
    ...
    users = relationship("User", secondary=user_tag_table, back_populates="tags")
...
class File(Base):
    ...
    users = relationship("User", secondary=user_file_table, back_populates="files")
